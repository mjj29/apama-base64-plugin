...
RUN g++-8 -std=c++17 -o $APAMA_WORK/lib/libBase64Plugin.so -I$APAMA_HOME/include -L$APAMA_HOME/lib -lapclient -I$APAMA_WORK/plugin -shared -fPIC $APAMA_WORK/plugin/Base64Plugin.cpp
RUN cp $APAMA_WORK/plugin/eventdefinitions/Base64Plugin.mon $APAMA_WORK/monitors/
